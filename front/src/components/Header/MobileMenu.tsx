    ...
    <>
      ...
      >
        ...
        <div className="px-4 py-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className={`overflow-hidden transition-[max-height] duration-300 ease-in-out ${profileOpen ? "max-h-60 mt-2" : "max-h-0"}`}>
            <div className="flex flex-col bg-white dark:bg-background-dark rounded-lg border border-black/10 dark:border-white/10">
              {!isLoggedIn ? (
                <>
                  <Link href="/login" className="px-4 py-2 text-black/80 dark:text-white/80 hover:bg-black/5 dark:hover:bg-white/5" onClick={() => setOpen(false)}>Log in / Sign Up</Link>
                </>
              ) : (
                <>
                  <Link href="/dashboard" className="px-4 py-2 text-black/80 dark:text-white/80 hover:bg-black/5 dark:hover:bg-white/5" onClick={() => setOpen(false)}>Dashboard</Link>
                  <button className="text-left w-full px-4 py-2 text-black/80 dark:text-white/80 hover:bg-black/5 dark:hover:bg-white/5" onClick={() => { logout(); setOpen(false); }}>Log Out</button>
                </>
              )}
            </div>
          </div>
        ...
        <nav className="flex flex-col p-4 gap-4">
          {navItems.map((item) => (
            <Link key={item.name} href={item.href} className="text-gray-700 dark:text-gray-300 hover:text-primary dark:hover:text-primary font-medium" onClick={() => setOpen(false)}>
              {item.name}
            </Link>
          ))}
        </nav>
